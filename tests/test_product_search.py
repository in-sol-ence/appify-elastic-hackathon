...
def hit():
 p=Product(product_id="real",manufacturer="Cytron",name="Driver",category="motor_driver",input_voltage_min_v=12,input_voltage_max_v=30,continuous_current_per_channel_a=20,control_interfaces=["PWM"],source_type="manufacturer_product_page",source_url="https://example.com/product");return {"hits":{"hits":[{"_score":3,"_source":p.model_dump(mode="json")|{"semantic_text":p.semantic_content()}}]}}
def bad_request():
