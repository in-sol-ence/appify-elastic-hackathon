 ...
 async with Actor:
  ...
  for source in sources[:min(int(data.get('maximum_requests',10)),10)]:
   ...
   if run_type=='product_discovery':
    result={'schema_version':'1.0','run_type':'product_discovery','project_id':data.get('project_id'),'component_role_id':data.get('component_role_id'),'candidate_identity':result.get('identity',{}),'candidate_specifications':{},'source_url':result.get('source_url'),'source_evidence':result.get('evidence',{}),'extraction':result.get('extraction',{}),'compatibility_status':'unevaluated'}
