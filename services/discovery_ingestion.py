...
def ingest_discovery_candidate(item,project,client=None):
 ...
 key=mpn if mpn else (f'{manufacturer}|{model}' if model else f"{manufacturer}|{name}");product_id='discovered-'+hashlib.sha256(key.lower().encode()).hexdigest()[:24]
 category=item.get('search_profile',{}).get('category') or 'unknown';specs=item.get('candidate_specifications') or {};commercial=item.get('candidate_commercial') or {}
 product=Product(product_id=product_id,manufacturer=manufacturer,name=name,model=model,manufacturer_part_number=mpn or '',category=category,description='Candidate discovered from an approved source; specifications require verification.',price_estimate=commercial.get('price'),currency=commercial.get('currency'),source_type='apify_discovery',source_url=item.get('source_url',''),specification_confidence=item.get('extraction',{}).get('confidence',0),**{k:v for k,v in specs.items() if k in Product.model_fields})
